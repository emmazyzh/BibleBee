...
export async function ensureClerkUsersTable(bindings) {
  ...
  await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS id TEXT DEFAULT (gen_random_uuid())::text`
  ...
  await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS clerk_created_at TIMESTAMPTZ`
...
export async function upsertClerkUser(user, bindings) {
  const sql = getSql(bindings)
...
export async function deleteClerkUser(clerkUserId, bindings) {
  const sql = getSql(bindings)
